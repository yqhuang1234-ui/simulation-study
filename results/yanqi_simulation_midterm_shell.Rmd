...
2.  The error term $\varepsilon_i$ is generated as: ($\mu_x$ is the mean of $x$)
...
```{r chunk-name, fig.cap="Table 1 ", echo=F}
library(dplyr)
...
These simulation results suggest that although OLS slope estimates remain unbiased under heteroscedasticity, inference based on model-based standard errors is unreliable. As shown in Figure 1 and Table 1, empirical variability increased as heteroscedasticity strengthened, while model-based standard errors decreased slightly. This mismatch produced confidence intervals with substantially reduced coverage, dropping nearly 20 percentage points below the nominal level under strong heteroscedasticity.

This indicates that when the underlying data are heteroscedastic, traditional OLS-based inference is unreliable. In particular, researchers relying on nominal confidence intervals may draw overly optimistic conclusions about the precision of their estimates. In practice, robust alternatives such as heteroscedasticity-consistent standard errors or alternative modeling approaches should be used to ensure valid inference.
